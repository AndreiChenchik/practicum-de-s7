...
df_joined = df.join(
    df_library, df.book_id == df_library.book_id, how="left_anti"
)

print(df_joined.select("title").explain())
print(df_joined.cache().explain())

spark.sparkContext.setCheckpointDir("/user/andrei/analytics/test_check")
print(df_joined.checkpoint().explain())

# Двигайтесь дальше! Ваш код: einWLzgTYR
